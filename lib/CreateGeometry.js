  ...
  for( var i = 0; i < size; i++ ){
    for( var j = 0; j < size; j++ ){
      ...
      for( var k = 0; k < depth-1; k++ ){
        ...
        positions[ pIndex + 2   ] = k/depth;

        uvs[ iIndex + 0 ]        = k/depth; 
        uvs[ iIndex + 1 ]        = 1;

        positions[ pIndex + 3   ] = i / size;
        positions[ pIndex + 4   ] = j / size;
        positions[ pIndex + 5   ] = k/depth;

        uvs[ iIndex + 2 ]        = k/depth; 
        uvs[ iIndex + 3 ]        = -1;


        positions[ pIndex + 6   ] = i / size;
        positions[ pIndex + 7   ] = j / size;
        positions[ pIndex + 8   ] = (k+1)/depth;

        uvs[ iIndex + 4 ]        = (k+1)/depth; 
        uvs[ iIndex + 5 ]        = 1;
        ...
        positions[ pIndex + 11  ] = (k+1)/depth;
  
        uvs[ iIndex + 6 ]        = (k+1)/depth; 
        uvs[ iIndex + 7 ]        = 1;

        positions[ pIndex + 12  ] = i / size;
        positions[ pIndex + 13  ] = j / size;
        positions[ pIndex + 14  ] = k/depth;

        uvs[ iIndex + 8 ]        = k/depth; 
        uvs[ iIndex + 9 ]        = -1;

        positions[ pIndex + 15  ] = i / size;
        positions[ pIndex + 16  ] = j / size;
        positions[ pIndex + 17  ] = (k+1)/depth;

        uvs[ iIndex + 10 ]       = (k+1)/depth; 
        uvs[ iIndex + 11 ]       = -1;
